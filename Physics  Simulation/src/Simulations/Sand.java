...
public class Sand extends JPanel implements Runnable{
    ...
    public Sand(JFrame frame, Menu menu) {
		this.frame = frame;
		this.menu = menu;
    ...
    public void addBall(float x, float y, int r, double angle, int speed, Color col) {
		...
		sand.add(new Ball(x, y, r, spx, spy, col));
	}
	...
	public void checkCollisions() { 
		...
		synchronized(sand) {
			Collections.sort(sand, (b1, b2) -> Float.compare(b1.x - b1.r, b2.x - b2.r));
		}
		
		List<List<Ball>> intervals = new ArrayList<>();
		
		//float left = sand.get(0).x - sand.get(0).r;
		float right = sand.get(0).x + sand.get(0).r;
		List<Ball> tem = new ArrayList<>();
		tem.add(sand.get(0));
		intervals.add(tem);
		
		for(int i=1;i<n;i++) {
			float newLeft = sand.get(i).x - sand.get(i).r;
			float newRight = sand.get(i).x + sand.get(i).r;
			if(newLeft <= right) {
				right = Math.max(right, newRight);
				intervals.get(intervals.size()-1).add(sand.get(i));
			}
			else {
				List<Ball> temp = new ArrayList<>();
				temp.add(sand.get(i));
				intervals.add(temp);
				right = sand.get(i).x + sand.get(i).r;
			}
	...
	public void paintComponent(Graphics graphics) {
		...
		synchronized(sand) { //synchronized so that 2 threads dont try to edit simultanuelsyle
			for(Ball ball : sand) {
				g.setColor(ball.col);
